...
let nextStartCheck = parseInt(PropertiesService.getScriptProperties().getProperty('nextStartCheck'));
if (!nextStartCheck) {
  nextStartCheck = 267;
}
...
console.log(`Starting loop with nextStartCheck = ${nextStartCheck} and responses.length = ${responses.length}`);
...
for (let i = nextStartCheck; i < responses.length; i++) {
  try {
    ...
    console.log(`processed = ${processed}, nb_exec_max = ${nb_exec_max}, (i + 1) = ${i + 1}, responses.length = ${responses.length}`);
    ...
    if (processed >= nb_exec_max || (i + 1) >= responses.length) {
      ...
      PropertiesService.getScriptProperties().setProperty('nextStartCheck', nextStartCheck); // Sauvegarder nextStartCheck
      break;
    }
  } catch (e) {
...
let lignesRestantes = false;
  ...
  if (nextStartCheck < responses.length) {
    ...
    PropertiesService.getScriptProperties().setProperty('nextStartCheck', nextStartCheck);
  }
...
const checkAppointmentExists = (newDeclare) => {
  ...
  for (let i = 9; i < newDeclare.length; i++) {
    ...
    if (cutTD != '') {
      ...
      const events = CalendarApp.getCalendarsByName(calendarName)[0].getEvents(rdvDeb, rdvFin);
      for (let event of events) {
        if (event.getTitle() === titre && event.getStartTime().getHours() === rdvDeb.getHours() && event.getEndTime().getHours() === rdvFin.getHours()
            && event.getStartTime().getMinutes() === rdvDeb.getMinutes() && event.getEndTime().getMinutes() === rdvFin.getMinutes()) {
